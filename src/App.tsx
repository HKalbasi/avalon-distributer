...
function App() {
    ...
    const gameSerializedBytes = pako.deflate(JSON.stringify([...friends.filter(a => a.is_in_game), {
        name: me,
        is_in_game: true
    }]));
    ...
    return (
        <div className="container text-center px-10 pt-2">
            <h1 className="text-4xl font-bold text-center bg-clip-text text-transparent bg-gradient-to-r from-primary to-secondary border-b-2 border-primary/20"
                onClick={() => setGameType(gameType === 'Avalon' ? 'Secret Hitler' : 'Avalon')}>{gameType}</h1>
            <div
                className="flex justify-between items-center rounded-md mt-2 px-4 py-2 ro border-2 border-secondary/50">
                <p>your name: {me}</p>
            ...
            <div className="my-4 border-b-2 border-primary/50 pb-2">
                <div className="flex flex-col gap-3">
                    ...
                    {showFriends && (<>
                            <button className="btn btn-secondary btn-dash"
                                    onClick={() => setFriendsPermanent([...friends, {
                                        name: window.prompt('Name?') ?? "gav",
                                        is_in_game: false
                                    }], Commands.Add)}>
                                Add new friend
                            ...
                            {friends.map((friend, i) => (
                                <div
                                    ...
                                    onClick={() =>
                                        setFriendsPermanent(
                                            friends.map((f, j) =>
                                                i === j ? {...f, is_in_game: !f.is_in_game} : f
                                            ),
                                            Commands.Toggle
                                        )
                                    }
                                >
                                    ...
                                    <svg
                                        onClick={(e) => {
                                            ...
                                            setFriendsPermanent(friends.filter((_, j) => i !== j), Commands.Remove);
                                        }}
            ...
            <div className="flex flex-col gap-5">
                        ...
                        {players.map((player) => (
                            <span
                                onClick={() =>
                                    setFriendsPermanent(
                                        friends.map((f) =>
                                            player === f.name ? {...f, is_in_game: !f.is_in_game} : f
                                        ),
                                        Commands.Toggle
                                    )
                                }
                                className="btn btn-dash">{player}
                            </span>
                        ))}
                    </div>
                </div>
                ...
                <div className="join">
                    <button className="btn join-item" onClick={() => setSeed(makeid(4))}>Game seed</button>
